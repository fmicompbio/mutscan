...
n <- 20L
alph <- c("A","C","G","T","N")
seqs <- unlist(lapply(seq_len(n - length(alph) + 1), function(i) paste(sample(alph, size = k, replace = TRUE), collapse = "")))
...
test_that("low-level BK tree functions work as expected", {
  ...
  # print tree
  expect_output(mutscan:::bk_print(), paste0("current size: ", n, ".+",
                                   n, ".+", seqs[1], ", 0"))
  ...
  # ground truth e.g. from: sum(stringdist::stringdist(seqs[1], seqs, method = "hamming") <= 24)
