...
collapseMutantsBySimilarity <- function(se, assayName, scoreMethod = "rowSum",
    ...
    if (scoreMethod == "rowSum") {
        scores <- rowSums(assay(se, assayName))
    } else if (scoreMethod == "rowMean") {
        scores <- rowMeans(assay(se, assayName))
    }
    ...
    matchTable$representative <- rownames(se)[
        match(matchTable$representative, 
              rowData(se)[, sequenceCol])]
    
    ## Add grouping variable
    rowData(se)$collapseCol <- 
        matchTable$representative[
            match(rowData(se)[, sequenceCol], 
                  matchTable$sequence)]
...
collapseMutants <- function(se, nameCol) {
    ...
    aList <- lapply(assays(se), function(a) {
        rowsum(
            x = a,
            group = factor(rowData(se)[[nameCol]])
        )
    })
    ...
    for (v in setdiff(
        intersect(c("mutantName", "mutantNameBase", "mutantNameBaseHGVS",
                  ...
                  colnames(rowData(se))),
        nameCol)) {
        tmp <- mergeValues(rowData(se)[[nameCol]],
                           rowData(se)[[v]])
    ...
    stopifnot(cn1 == rownames(colData(se)))
